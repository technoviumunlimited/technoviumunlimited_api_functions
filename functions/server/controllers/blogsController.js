    ...
    if (category_id) {
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (categories.some((category) => category.category_id === category_id)) {
          ...
          filteredBlogs.push({ _id: doc.id, ...data });
        }
      });
...
//     const page = req.query.page ? parseInt(req.query.page) : 1;
//     const itemsPerPage = req.query.take ? parseInt(req.query.take) : 10;
//     const category_ids = req.query.category_ids ? req.query.category_ids : "";
...
//       query.where("category", "array-contains-any", [{ category_id: category_ids }])
//       .whereArrayContains("category.$.category_id", category_ids);
      
//       //query.where("category", "$elemMatch", {     category_id: category_ids   });
